<?php
// Databaseverbinding importeren
require "db-accounts.php";

// Specifiek coin-ID instellen (in dit voorbeeld: Bitcoin met ID 1)
$coinId = 1;

// Coingegevens ophalen uit de database
$coin = $pdo->prepare("SELECT * FROM coins WHERE id = ?");
$coin->execute([$coinId]);
$coin = $coin->fetch(PDO::FETCH_ASSOC);

// Als de coin niet bestaat, stoppen we de pagina
if (!$coin) { exit("Coin not found"); }

// Arrays om fouten en ingevulde data op te slaan
$errors = [];
$inputs = [];

// Als het formulier verzonden is
if (isset($_POST['submit'])) {
    // Ingevoerde data filteren en valideren
    $name = filter_input(INPUT_POST, 'name', FILTER_SANITIZE_SPECIAL_CHARS);
    $review = filter_input(INPUT_POST, 'review', FILTER_SANITIZE_SPECIAL_CHARS);
    $rating = filter_input(INPUT_POST, 'rating', FILTER_VALIDATE_INT);

    // Validatie: naam moet ingevuld zijn
    if (!$name) $errors['name'] = "Enter your name.";
    else $inputs['name'] = $name;

    // Validatie: reviewtekst moet ingevuld zijn
    ...
    // Als er geen fouten zijn, voeg de review toe aan de database
    if (!$errors) {
        $stmt = $pdo->prepare("INSERT INTO reviews (name, review, rating, coin_id) VALUES (?, ?, ?, ?)");
        $stmt->execute([$name, $review, $rating, $coinId]);

        // Na succesvolle invoer herladen we de pagina (om dubbele verzending te voorkomen)
        header("Location: bitcoin.php");
        exit;
    }
}

// Alle reviews ophalen voor de huidige coin
